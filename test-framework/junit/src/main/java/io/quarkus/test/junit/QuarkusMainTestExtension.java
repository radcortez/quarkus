...
public class QuarkusMainTestExtension extends AbstractJvmQuarkusTestExtension
    ...
    protected PrepareResult prepare(ExtensionContext context) throws Exception {
        ...
        if (testProfile.isPresent()) {
            TestProfileSource testProfileSource = TestProfileAndProperties.createTestProfileSource(testProfile.get());
            additionalPaths.add(testProfileSource.getPropertiesLocation());
            shutdownTasks.add(testProfileSource.closeTask());
        }
    ...
    private int doJavaStart(ExtensionContext context, String[] arguments) throws Exception {
        ...
        try {
            ...
            Optional<QuarkusTestProfile> profileInstance = prepareResult.profile();
